def validations(input_base, digits, output_base):
    if input_base <= 1:
...
def convert_to_decimal(digits, input_base):
    ...
    for power, digit in enumerate(reversed(digits)):
        # print(power, digit)
        number_in_decimal += digit * input_base ** power
    
    return number_in_decimal
        

def decimal_to_new_base(number, output_base):
    ...
    dividend = number // output_base
    rest = number % output_base
    output_number.append(rest)

    while dividend >= output_base:
        rest = dividend % output_base
        output_number.append(rest)
        dividend //= output_base

    output_number.append(dividend)
...
def rebase(input_base, digits, output_base):
    ...
    validations(input_base, digits, output_base)

    number_base_10 = convert_to_decimal(digits, input_base)
    if output_base == 10:
        # Convert the result to a list of strings
        desired_number = list((str(number_base_10)))
        # Convert them back to a list of int.
        desired_number = list(map(int, desired_number))
    else:
        desired_number = decimal_to_new_base(number_base_10, output_base)
